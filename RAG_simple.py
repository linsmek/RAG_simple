...
def add_to_vector_collection(all_splits: list[Document], file_name: str, space: str, backend: str):
    ...
    st.sidebar.success(f"Data from {file_name} added to the {backend} vector store!")
...
def call_llm(context: str, prompt: str, history: list[dict], temperature: float):
    """
    Calls the Ollama LLM, using the specified `temperature` to control the 
    creativity or randomness of the generated response.
    """
    history_text = "\n\n".join(
    ...
    llm = Ollama(
        ...
        temperature=temperature,           # Include the temperature here
    )
...
def main():
    ...
    with st.sidebar:
        ...
        uploaded_files = st.file_uploader(
            "**📄 Upload PDF Files for Chatbot**", 
            type=["pdf"], 
            accept_multiple_files=True
        )
        process = st.button("⚡️ Process All Documents")
    ...
    st.markdown("Ask questions related to your uploaded documents and receive detailed answers.")
    ...
    if user_input:
        ...
        results = query_collection(user_input, space, backend)
        context_docs = results.get("documents", [[]])[0]

        if not context_docs:
            assistant_response = f"No documents available in {backend}. Please upload and process PDFs first."
        else:
            # Concatenate the retrieved document texts as context
            ...
            st.session_state.history.append({"question": user_input, "answer": assistant_response})

        # Display assistant response
        st.session_state.messages.append({"content": assistant_response, "is_user": False})
        st.chat_message("assistant").write(assistant_response)

    # Optional: Display search history and retrieved documents
